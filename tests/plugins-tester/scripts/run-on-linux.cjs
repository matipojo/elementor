...
async function run( args ) {
	...
	const commandToRun = `/bin/bash -c "npm run test:plugins-tester:prepare && npm run test:plugins-tester"`;

	spawn( `${ command } ${ options.join( ' ' ) } ${ image } ${ commandToRun } tail -f /dev/null`, {
		stdio: 'inherit',
		stderr: 'inherit',
		shell: true,
	} );
